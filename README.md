...
**亲和性（Affinity）**：非 auto 模式下按 **turn → session → user** 三级键优先复用上次成功使用的渠道。turn 层取轮级恒定头（`X-Conversation-Request-Id` → `X-Query-Id` → `X-Root-Request-Id`，取不到不生成随机 id）；这三个头在主流编程 agent 上均未见实际发送，因此当真实头全未命中且存在 session 锚点时，改从请求体**无状态派生** turn id（`sha256(sessionID + 最后一条真实 user 消息下标 + 文本指纹)`，轮内恒定、跨轮递增），由 `AFFINITY_DERIVE_TURN_ID` 控制（默认开启，置 `false` 一键回退旧行为）。session 层取会话级头（`conversation_id` → `session_id` → `X-Conversation-Id` → `X-Session-Id` → `Session-Id` → `Thread-Id` → `X-Parent-Session-Id` → `X-Claude-Code-Session-Id` → `X-Opencode-Session` → `X-Litellm-Session-Id` → `Acp-Connection-Id` → `Acp-Session-Id`），都无则降级 `userId` 兜底（旧行为）——但该 user 兜底层仅服务无会话标识的客户端：`SessionID` 非空时读写都不产出 user 键，避免同一用户的所有会话被吸到同一渠道（削热点），由 `AFFINITY_SESSION_EXCLUDES_USER` 控制（默认开启，仅 `false` 关闭、关闭即回退旧行为，启动期只读）。预期副作用：有 session 的新会话首请求由「user 命中」变为 miss（随后加权随机散开并绑定自己的 session 键），`AFFINITY_STATS_INTERVAL_SECONDS` 汇总日志中 miss 计数上升、user 命中下降，属预期效果而非回归。会话标识优先取请求头，全部未命中时再尝试从 JSON 请求体补取（`litellm_session_id` / `session_id` / `conversation_id` / `metadata.cline_task_id`，取首个非空），仅限 JSON 的 POST/PUT/PATCH，由 `AFFINITY_BODY_SESSION_ID` 控制（默认开启，置 `false` 关闭）。三层 TTL 分别为 120 / 1800 / 300 秒（`AFFINITY_EXPIRE_SECONDS`=300 为用户层）。`X-Request-Id` / `X-Client-Request-Id` 的语义随客户端而变（`X-Client-Request-Id` 在 Codex CLI 下实为 thread id），不可假定为轮级，故一律不收作 turn 键。亲和生效率可通过 `AFFINITY_STATS_INTERVAL_SECONDS` 汇总日志观测（每窗口输出 turn / 派生 turn / session / user 命中、fallback、miss 六档分布及未命中候选头名采样）；注意命中不含「键命中但渠道不在候选集、已回落加权随机」的 fallback 档，否则会高估改造效果。
...
**熔断器**：基于 EWMA 算法跟踪每个账号的错误率。当错误率达到 50% 或连续失败 5 次时自动熔断该账号。后台探针在熔断后持续探测，连续 3 次成功后自动恢复。客户端请求形态类失败（参数/格式非法等）不记入 EWMA、不触发熔断，与渠道冷却同一口径。
...
此外还支持**模型等价组（canonical name）**归一化匹配：为模型配置 `canonical_name` 后，该模型与其等价主名在渠道精确匹配时视为同一模型，便于将不同渠道对同一模型的命名差异归并到统一主名；未配置时匹配行为与原来完全一致。该字段在管理后台"模型元数据"页面（仅管理员可见，路由 `/metadata`）配置，直接指向主名、不支持链式传递；请求命中后仍转发各渠道 Models 里配置的真实模型名，倍率按实际转发名计算。
...
Dashboard 从「按日 × 模型」扩展为**四维度 × 三粒度**的统计看板，通过 `GET /api/user/dashboard/aggregate`（聚合行）与 `GET /api/user/dashboard/summary`（KPI 汇总）两个接口供数：

- **分区维度**：概览 / 模型分析 / 令牌分析对所有登录用户可见；渠道分析 / 用户分析**仅管理员可见**（普通用户访问受限维度返回 403，且模型 / 令牌维度强制限定为本人日志；管理员可传 `username` 精确过滤，留空为全部用户）
- **时间粒度**：支持小时 / 天 / 周（周取该周周一），时间范围提供 1/7/14/29 天预设与自定义
- **KPI 汇总**：总请求 / 总额度 / 总 Token、缓存命中率（`cached_tokens / prompt_tokens`）、平均首字延迟 / 平均耗时、平均 RPM / TPM；无延迟正样本时延迟显示 `--`
- **三视图**：每个分析分区支持趋势（折线）/ 占比（饼图）/ Top 排行（横向柱状图）切换，共用同一份数据、切换不重新请求；Top-N 可选 5/10/20/50（默认 10）
- **偏好记忆**：每个维度的图表类型与 Top-N 按维度持久化到浏览器 localStorage，重开页面自动恢复
- **模型名归一化**：模型维度自动归并厂商前缀、大小写与等价别名；渠道 / 令牌 / 用户维度的键逐字保留，大小写不同即独立统计
...
| `AFFINITY_DERIVE_TURN_ID` | 真实 turn 头全未命中且存在 session 锚点时，从请求体确定性派生 turn id（`sha256(sessionID + 最后一条真实 user 消息下标 + 文本指纹)`，轮内恒定、跨轮递增），使只有会话头的客户端也能获得 turn 层亲和收益；仅限 JSON 的 POST/PUT/PATCH；置 `false` 一键回退旧行为 | `true`（开启） |
| `AFFINITY_SESSION_EXCLUDES_USER` | 有 session 标识时读写都不含 user 层（user 层仅服务无会话标识的客户端，避免同一用户的所有会话被吸到同一渠道）；仅 `false` 关闭（忽略大小写与首尾空白），关闭即回退旧行为；启动期只读、不支持热更新 | `true`（开启） |
