...
func TestDiagnoseV2(t *testing.T) {
	...
	for i, chunk := range chunks {
		evts, convErr := codex.ConvertOpenAIChatToResponsesWithContext([]byte(firstLine), nil, []byte(chunk), &param, false)
		if convErr != nil {
			t.Fatalf("chunk %d produced conversion error: %v", i+1, convErr)
		}
		allEvents = append(allEvents, evts...)
		if len(evts) > 0 {
			fmt.Printf("[chunk %d] -> %d events\n", i+1, len(evts))
		}
	}
	...
	for i, evt := range allEvents {
		...
		if tt != "tool_search_call" && tt != "web_search_call" && tt != "custom_tool_call" && tt != "function_call" {
			continue
		}
		fmt.Printf("--- [%d] %s ---\n", i+1, et)
		var m map[string]interface{}
		json.Unmarshal([]byte(it.Raw), &m)
	...
	for _, evt := range allEvents {
		...
		if out.IsArray() && len(out.Array()) > 0 {
			...
			fmt.Println("  OUTPUT EMPTY OR MISSING")
		}
	}
	...
	if len(cl) >= 55 {
		...
		if gjson.Valid(d50) {
			...
			fmt.Printf("  line 51 INVALID JSON: %.80s\n", d50[:min(80, len(d50))])
		}
		...
		if gjson.Valid(d52) {
			...
			fmt.Printf("  line 53 INVALID JSON: %.80s\n", d52[:min(80, len(d52))])
		}
			...
			if out.IsArray() {
				...
				for j, it := range out.Array() {
					...
					if it.Get("type").String() == "tool_search_call" {
						fmt.Printf(" args=%s", it.Get("arguments").Raw)
					}
					fmt.Println()
				}
			}
		} else {
			fmt.Printf("  line 55 INVALID JSON: %.80s\n", d54[:min(80, len(d54))])
		}
		...
		if ourAdded != nil {
			...
			fmt.Println("  *** WE PRODUCED NO tool_search_call output_item.added ***")
		}
		...
		if ourDone != nil {
			...
			if gjson.Valid(d52) {
				...
				fmt.Printf("    codex args=%s, our args=%s\n", ca, oa)
				if ca != oa {
					fmt.Println("    *** DIFFERENT arguments ***")
				}
...
func extractDataStr(evt string) string {
	...
	if dIdx >= 0 {
		return strings.TrimSpace(evt[dIdx+6:])
	}
	return ""
}
