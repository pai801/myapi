...
**アフィニティ**: 非 auto モードでは、前回成功したチャネルを **turn → session → user** の階層キーで優先的に再利用します。turn 層は 1 回の送信内で一定のヘッダ（`X-Conversation-Request-Id` → `X-Query-Id` → `X-Root-Request-Id`；存在しなければランダム id は生成しない）を使用します。これら 3 つのヘッダは主要なコーディング agent では実際には送信されていないため、実ヘッダがすべて取得できず session アンカーが存在する場合は、代わりにリクエストボディから turn id を**ステートレスに派生**します（`sha256(sessionID + 最後の実 user メッセージのインデックス + テキスト指紋)`；turn 内では一定、turn をまたぐと増加）。これは `AFFINITY_DERIVE_TURN_ID`（デフォルト有効、`false` で旧動作にフォールバック）で制御されます。session 層は会話レベルのヘッダ（`conversation_id` → `session_id` → `X-Conversation-Id` → `X-Session-Id` → `Session-Id` → `Thread-Id` → `X-Parent-Session-Id` → `X-Claude-Code-Session-Id` → `X-Opencode-Session` → `X-Litellm-Session-Id` → `Acp-Connection-Id` → `Acp-Session-Id`）を使用し、いずれも存在しない場合は `userId` にフォールバックします（旧動作）。ただしこの user フォールバック層はセッション識別子を持たないクライアントのみに提供されます。`SessionID` が非空の場合、読み書きともに user 層を含めません。これにより同一ユーザーのすべてのセッションが同じチャネルに引き寄せられるのを防ぎます（ホットスポットの抑制）。これは `AFFINITY_SESSION_EXCLUDES_USER`（デフォルト有効、`false` のみ無効、無効にすると旧動作に戻る、起動時のみ読み取り）で制御されます。想定される副作用：session id を持つ新規セッションの初回リクエストは「user ヒット」から miss に変わり（その後は加重ランダムで散らばり、自身の session キーにバインドされます）、`AFFINITY_STATS_INTERVAL_SECONDS` サマリログでは miss カウントが増加し user 命中が減少しますが、これは想定内の効果でありリグレッションではありません。セッション識別子はまずリクエストヘッダから取得し、ヘッダがすべて取得できない場合は JSON リクエストボディから補完取得します（`litellm_session_id` / `session_id` / `conversation_id` / `metadata.cline_task_id` のうち最初に非空のもの；JSON の POST/PUT/PATCH のみ）。スイッチ `AFFINITY_BODY_SESSION_ID`（デフォルト有効、`false` で無効）で制御されます。3 層の TTL はそれぞれ 120 / 1800 / 300 秒です（`AFFINITY_EXPIRE_SECONDS`=300 はユーザー層）。`X-Request-Id` / `X-Client-Request-Id` の意味はクライアントによって異なり（`X-Client-Request-Id` は Codex CLI では実際には thread id）、turn 相当とは限らないため、いずれも turn キーとしては使いません。アフィニティの実効率は `AFFINITY_STATS_INTERVAL_SECONDS` サマリログで観測できます（各ウィンドウで turn / 派生 turn / session / user の命中・fallback・miss の 6 区分分布と、未命中時のヘッダ名サンプリングを出力）。ただし命中には「キーは一致したがチャネルが候補集合になく加重ランダムにフォールバックした」fallback 区分は含まれません。これを命中に算入すると改造効果を過大評価します。
...
| `AFFINITY_DERIVE_TURN_ID` | 実 turn ヘッダがすべて取得できず session アンカーが存在する場合、リクエストボディから turn id をステートレスに派生する（`sha256(sessionID + 最後の実 user メッセージのインデックス + テキスト指紋)`；turn 内では一定、turn をまたぐと増加）。これにより会話ヘッダしか送らないクライアントも turn 層アフィニティの恩恵を受けられる。JSON の POST/PUT/PATCH のみ。`false` で旧動作にフォールバック | `true`（有効） |
| `AFFINITY_SESSION_EXCLUDES_USER` | セッション識別子が存在する場合、読み書きともに user 層を含めない（user 層はセッション識別子を持たないクライアントのみを対象とし、同一ユーザーのすべてのセッションが同じチャネルに引き寄せられるのを防ぐ）。`false` のみ無効（大文字小文字と前後空白を無視）、無効にすると旧動作に戻る。起動時のみ読み取り、ホットリロード不可 | `true`（有効） |
